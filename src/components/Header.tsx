...
export function Header() {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-40 w-full max-w-full transition-all duration-300 ${
        scrolled
    ...
    >
      <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link to={localizedPath("/")} className="flex shrink-0 items-center gap-2 sm:gap-2.5">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-emerald-400 to-teal-600 shadow-lg shadow-emerald-500/20">
            <Smartphone className="h-5 w-5 text-white" />
          </div>
          <div className="flex flex-col leading-none">
            <span className="text-base font-bold text-white">GhostSMS</span>
            <span className="hidden text-[10px] font-medium uppercase tracking-wider text-emerald-400 sm:inline">Receive SMS Online</span>
          </div>
        </Link>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-lg border border-zinc-700 bg-zinc-800/80 text-zinc-200 xl:hidden"
          >
